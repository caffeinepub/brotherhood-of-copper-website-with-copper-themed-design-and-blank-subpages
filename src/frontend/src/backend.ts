...
export class Backend implements backendInterface {
    ...
    async getPosters(): Promise<Array<ExternalBlob>> {
        if (this.processError) {
            try {
                const result = await this.actor.getPosters();
    ...
    async uploadPoster(arg0: ExternalBlob): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.uploadPoster(await to_candid_ExternalBlob_n15(this._uploadFile, this._downloadFile, arg0));
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
